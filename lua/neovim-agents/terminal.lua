-- Terminal management for neovim-agents plugin
--
...
local function create_terminal_instance(id, config, agent_type)
  -- Use default agent if not specified
  agent_type = agent_type or config.default_agent
  ...
  if not terminals[id] then
    terminals[id] = {
      ...
      job_id = nil,
      id = id,
      agent_type = agent_type,  -- NEW: track which agent CLI this is
    }
  else
    -- Update agent_type if terminal is being recreated
    terminals[id].agent_type = agent_type
  end
  ...
  -- Get the command for this agent type
  local command = config.agents[agent_type].command
  ...
  vim.api.nvim_buf_set_keymap(term.buf, 't', '<C-n>', '<C-\\><C-n>:lua require("neovim-agents").new_terminal_from_terminal_handler()<CR>', {
    noremap = true,
    silent = true,
  ...
  vim.api.nvim_buf_set_keymap(term.buf, 't', '<C-r>', '<C-\\><C-n>:lua require("neovim-agents").rename_terminal_handler()<CR>', {
    noremap = true,
    silent = true,
  ...
  vim.api.nvim_buf_set_keymap(term.buf, 't', '<C-t>', '<C-\\><C-n>:lua require("neovim-agents").select_terminal_handler()<CR>', {
    noremap = true,
    silent = true,
...
local function create(config, agent_type)
  return create_terminal_instance(default_id, config, agent_type)
end

-- Toggle terminal visibility
function M.toggle(config, id, agent_type)
  id = id or active_id or default_id
