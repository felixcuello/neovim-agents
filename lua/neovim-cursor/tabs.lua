...
-- @param agent_type (optional) Agent type (cursor, claude, gemini, etc.)
-- @return terminal_id The ID of the created terminal
function M.create_terminal(name, config, agent_type)
  ...
  state.terminals[id] = {
    id = id,
    name = name,
    agent_type = agent_type,  -- NEW: store agent type
    created_at = os.time(),
    last_active = os.time(),
  }
  
  -- Create the actual terminal instance
  terminal._create_terminal_instance(id, config, agent_type)
...
function M.switch_to(id, config)
  ...
  if terminal.is_running(id) then
    ...
    -- Terminal died, recreate it with the same agent type
    local agent_type = state.terminals[id].agent_type
    terminal._create_terminal_instance(id, config, agent_type)
  end
